# Wi-Fi Speed Monitor V2 - Ookla CLI Experiment

Versi 2 ini dipisahkan dari versi utama untuk eksperimen backend speedtest.
Selain backend bawaan `speedtest-cli`, versi ini bisa memakai **Ookla official
CLI** jika executable resmi Ookla tersedia di komputer.
...
- Pilihan backend speedtest:
  - `speedtest_cli`: library Python yang dipakai versi utama.
  - `ookla_cli`: Ookla official CLI eksternal, jika tersedia.
...
File `.exe` bisa dibuat otomatis oleh GitHub Actions pada workflow `Build Windows EXE V2`.
Cara mengambilnya:

1. Buka tab **Actions** di repository GitHub.
2. Pilih workflow **Build Windows EXE** yang terbaru.
3. Download artifact **WiFiSpeedMonitorV2-Windows**.
4. Extract file zip, lalu jalankan `WiFiSpeedMonitorV2.exe`.
...
Hasilnya ada di `package\WiFiSpeedMonitorV2.exe`.
...
Pada versi 2, field `speedtest_backend` bisa diubah ke `ookla_cli`. Isi
`ookla_cli_path` dengan path executable Ookla official CLI, misalnya:
...
Jika aplikasi dijalankan dari `WiFiSpeedMonitorV2.exe`, scheduler Windows akan
